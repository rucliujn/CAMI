...
import ProductSearch.PersonalizedEmbedding_mult as PersonalizedEmbedding
#import PersonalizedEmbedding

class ProductSearchEmbedding_model(object):
	def __init__(self, data_set, window_size,
				 embed_size, mult_size, div_lambda, max_gradient_norm, batch_size, learning_rate, L2_lambda, dynamic_weight,
				 query_weight, net_struct, similarity_func, path, forward_only=False, negative_sample = 5):
		"""Create the model.
		...
		Args:
			...
			mult_size: the size of interest
		...
		self.query_word_masks = tf.placeholder(tf.float32, shape=[None, self.query_max_length], name="query_word_masks")
		self.learning_rate = tf.placeholder(tf.float32, name="learning_rate")
		self.tau = tf.placeholder(tf.float32, name="tau")
		init_width = 0.5 / self.embed_size
		'''
		def read_embeddings(path, name, shape) :
			if (name in ['user', 'product', 'word', 'related_product', 'brand', 'categories']) :
				embedding_file = path + name + ".txt"
				...
				for line in f :
					...
					if (count <= 2) :
						if (count == 1) :
							...
							length = int(line)
							embedding_array = np.zeros((size, length))
					else :
						embedding = line.strip().split(' ')
						embedding = [float(x) for x in embedding]
						embedding = np.array(embedding)
						embedding_array[count - 3, :] = embedding
				embedding_array = tf.convert_to_tensor(embedding_array)
			else :
				embedding_array = tf.random_uniform(shape)
			embedding_array = tf.cast(embedding_array, tf.float32)
			return embedding_array
		'''
		def get_cates(name, cate_id) :
			if (name == 'user') :
				return tf.cast(tf.convert_to_tensor(np.array(list(range(len(data_set.category_ids) + 1, len(data_set.category_ids) + data_set.user_size * self.mult_size + 1)), dtype=np.int32)), tf.int32)
			else :
				return tf.cast(tf.convert_to_tensor(np.array(cate_id, dtype=np.int32)), tf.int32)
		...
		def entity(name, vocab):
			...
			return {
				...
				'embedding' : tf.Variable(
								tf.random_uniform([len(vocab) + 1, self.embed_size * self.mult_size if (name == 'user') else self.embed_size], -init_width, init_width) if (name != 'categories') 
								else tf.concat([tf.random_uniform([len(vocab) + 1, self.embed_size], -init_width, init_width),tf.random_uniform([data_set.user_size * self.mult_size, self.embed_size], -init_width, init_width)], axis = 0),
								name="%s_emb"%name)
							}
		self.entity_dict = {
		...
		def category(name, vocab) :
			print('%s size %s' % (name, str(len(vocab))))
			return {
				'name' : name,
				'vocab' : vocab,
				'size': len(vocab), 
				'embedding' : tf.Variable(get_cates(name, vocab), dtype=tf.int32,  trainable=False)
			}
		
		self.cate_dict = {
			'user' : category('user', list(range(self.mult_size * data_set.user_size))),
			'product' : category('product', data_set.product_cate_ids)
		}
		...
		if not forward_only:
			...
			# user + query -> product
			self.product_scores = PersonalizedEmbedding.get_product_scores(self, self.user_idxs, self.query_word_idxs, self.query_word_masks)
			#self.product_scores, cate_ids, cate_scores = PersonalizedEmbedding.get_product_scores(self, self.user_idxs, self.query_word_idxs, self.query_word_masks)
			#self.product_scores = tf.stack([self.product_scores, tf.cast(cate_ids, tf.float32), cate_scores], 2)
			with variable_scope.variable_scope("embedding_graph"):
				product_cate_word_idxs = self.data_set.category_words
				product_cate_word_masks = self.data_set.category_word_masks
				self.cate_emb, _ =  PersonalizedEmbedding.get_query_embedding(self, product_cate_word_idxs, product_cate_word_masks, self.entity_dict['word']['embedding'], reuse=True, scope=None)
	...
	def step(self, session, input_feed, forward_only, test_mode = 'product_scores'):
		...
		if not forward_only:
			...
			if test_mode == 'output_embedding':
				...
				self.embed_output_keys.append('user_cate')
				output_feed.append(self.cate_dict['user']['embedding'])
				
				self.embed_output_keys.append('cate_emb')
				output_feed.append(self.cate_emb)
			else:
	...
	def get_train_batch(self):
		...
		tau = self.min_tau + (self.max_tau - self.min_tau) * (1.0 - self.finished_word_num / self.words_to_train)
		#tau = max(self.max_tau * math.pow(self.min_tau / self.max_tau, self.finished_word_num / self.words_to_train), self.min_tau)
		...
		while len(word_idxs) < self.batch_size:
			...
			if self.data_set.sub_sampling_rate == None or random.random() < self.data_set.sub_sampling_rate[text_list[self.cur_word_i]]:
				...
				query_word_masks.append(self.data_set.query_word_masks[query_idx])
		...
		input_feed[self.query_word_masks.name] = query_word_masks
	...
	def get_test_batch(self):
		...
		# Input feed: encoder inputs, decoder inputs, target_weights, as provided.
		input_feed = {}
		input_feed[self.tau.name] = self.min_tau
		input_feed[self.learning_rate.name] = learning_rate
		input_feed[self.user_idxs.name] = user_idxs
		input_feed[self.product_idxs.name] = product_idxs
		input_feed[self.query_word_idxs.name] = query_word_idxs
