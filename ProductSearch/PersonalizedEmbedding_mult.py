...
from interest_selection import get_scores, get_true_scores
																	
def get_product_scores(model, user_idxs, query_word_idx, query_word_mask, product_idxs = None, scope = None):
	with variable_scope.variable_scope(scope or "embedding_graph"):										
		...
		user_lambda = tf.nn.embedding_lookup(model.user_lambda, user_idxs)
		user_p = tf.nn.embedding_lookup(model.user_p, user_idxs)
		# get query embedding [None, embed_size]
		if model.dynamic_weight >= 0.0:
			print('Query as a dynamic relationship')
			query_vec, query_embs = get_query_embedding(model, query_word_idx, query_word_mask, model.entity_dict['word']['embedding'], reuse=True, scope=None)
		else:
			print('Query as a static relationship')
			query_vec = model.query_static_vec
		print('Query size :', query_vec.shape)
		...
		user_size = array_ops.shape(user_vec)[0]#get user_size		
												
		print('Similarity Function : ' + model.similarity_func)										
		#example_vec = (1.0 - model.Wu) * user_vec + model.Wu * query_vec
		
		user_multi_vec = tf.reshape(user_vec, [-1, model.mult_size, model.embed_size])#user_size, K, embed_size
		query_multi_vec = tf.reshape(tf.tile(query_vec, [1, model.mult_size]), [-1, model.mult_size, model.embed_size])#user_size, K, embed_size

		user_cate_idxs = tf.cast(tf.convert_to_tensor(np.array(list(range(model.mult_size)), dtype=np.int64)), tf.int64)
		user_cate_idxs = tf.tile(user_cate_idxs, [user_size])
		user_cate_idxs = user_cate_idxs + model.mult_size * tf.reshape(tf.tile(tf.expand_dims(user_idxs, 1), [1, model.mult_size]), [-1])
		
		user_cate = tf.nn.embedding_lookup(model.entity_dict['categories']['embedding'], tf.nn.embedding_lookup(model.cate_dict['user']['embedding'], user_cate_idxs))
		user_cate = tf.reshape(user_cate, [-1, model.mult_size, model.embed_size])#user_size, K, embed_size
		if (product_idxs != None) :
			product_cate_idxs = tf.nn.embedding_lookup(model.cate_dict['product']['embedding'], product_idxs)
			product_cate_word_idxs = tf.gather(model.data_set.category_words, product_cate_idxs)
			product_cate_word_masks = tf.gather(model.data_set.category_word_masks, product_cate_idxs)
			product_cate_emb, _ = get_query_embedding(model, product_cate_word_idxs, product_cate_word_masks, model.entity_dict['word']['embedding'], reuse=True, scope=None) # get cate vectors
			product_cate = product_cate_emb
		else :
			product_cate_idxs = model.cate_dict['product']['embedding']
			product_cate_word_idxs = tf.gather(model.data_set.category_words, product_cate_idxs)
			product_cate_word_masks = tf.gather(model.data_set.category_word_masks, product_cate_idxs)
			product_cate_emb, _ = get_query_embedding(model, product_cate_word_idxs, product_cate_word_masks, model.entity_dict['word']['embedding'], reuse=True, scope=None) # get cate vectors
			product_cate = product_cate_emb
		example_multi_vec = user_multi_vec + query_multi_vec#user_size, K, embed_size
		attention_weight = tf.reduce_sum(query_multi_vec * user_cate, 2, keep_dims=False)#batch, K

		example_multi_vec = tf.reshape(example_multi_vec, [-1, model.embed_size])#user*K ,embed_size
		user_cate = tf.reshape(user_cate, [-1, model.embed_size])

		return get_scores(model, example_multi_vec, product_vec, product_bias, user_lambda, user_p, 'hard' if model.selection == 'gumbel' else model.selection, user_cate, product_cate, attention_weight)
...
def get_fs_from_words(model, word_vecs, query_len, reuse, scope=None):
	with variable_scope.variable_scope(scope or 'f_s_abstraction',
										 reuse=reuse):
		# get mean word vectors
		mean_word_vec = tf.reduce_sum(word_vecs, 1) / query_len
		# get f(s)
...
def get_query_embedding(model, word_idxs, word_masks, word_emb, reuse, scope = None):
	query_len = tf.tile(tf.reduce_sum(word_masks, 1, keep_dims=True), [1, model.embed_size])
	word_masks = tf.tile(tf.expand_dims(word_masks, 2), [1, 1, model.embed_size])
	word_vecs = tf.nn.embedding_lookup(word_emb, word_idxs) * word_masks
	if 'mean' in model.net_struct: # mean vector
		...
		return get_fs_from_words(model, word_vecs, query_len, reuse, scope)
...
def build_graph_and_loss(model, scope = None):											
	with variable_scope.variable_scope(scope or "embedding_graph"):	
		...
		if model.dynamic_weight >= 0.0:
			...
			query_vec, qw_embs = get_query_embedding(model, model.query_word_idxs, model.query_word_masks, model.entity_dict['word']['embedding'], reuse=None, scope=None) # get query vector
		...
		model.user_lambda = tf.Variable(tf.ones([model.entity_dict['user']['size'] + 1]), name="user_l")
		model.user_p = tf.Variable(tf.ones([model.entity_dict['user']['size'] + 1]), name="user_p")
		uqr_loss, uqr_embs  = pair_search_loss(model, model.Wu, query_vec, query_vec, model.user_idxs, # product prediction loss
							model.entity_dict['user']['embedding'], model.product_idxs, 
							model.entity_dict['product']['embedding'], model.product_bias, 
							len(model.entity_dict['product']['vocab']), model.data_set.product_distribute, model.user_lambda, model.user_p, dynamic=model.mult) 			
		...
		user_cate_idxs = tf.cast(tf.convert_to_tensor(np.array(list(range(model.mult_size)), dtype=np.int64)), tf.int64)
		user_cate_idxs = tf.tile(user_cate_idxs, [batch_size])
		user_cate_idxs = user_cate_idxs + model.mult_size * tf.reshape(tf.tile(tf.expand_dims(model.user_idxs, 1), [1, model.mult_size]), [-1])

		user_cate = tf.nn.embedding_lookup(model.entity_dict['categories']['embedding'], tf.nn.embedding_lookup(model.cate_dict['user']['embedding'], user_cate_idxs))
		user_cate = tf.reshape(user_cate, [-1, model.mult_size, model.embed_size])#batch, K, d
		user_cate_sim = tf.matmul(user_cate, user_cate, transpose_b=True)#batch, K, K
		user_cate_length = tf.sqrt(tf.reduce_sum(tf.square(user_cate), axis=2, keep_dims=True))#batch, K, 1
		user_cate_length = tf.matmul(user_cate_length, user_cate_length, transpose_b=True)#batch, K, K
		user_cate_sim = user_cate_sim / user_cate_length#batch, K, K
		div_cate_loss = tf.reduce_sum(tf.abs(user_cate_sim))

		loss += model.div_lambda * div_cate_loss
...
def relation_nce_loss(model, add_weight, example_idxs, head_entity_name, relation_name, tail_entity_name):
	...
	loss, embs = pair_search_loss(model, add_weight, relation_vec, relation_vec, example_idxs, example_emb, label_idxs, label_emb, label_bias, label_size, label_distribution, None, None, dynamic=False)
...
def pair_search_loss(model, add_weight, relation_vec, relation_cate_vec, example_idxs, example_emb, label_idxs, label_emb, label_bias, label_size, label_distribution, user_lambda, user_p, dynamic):
  ...
  # Nodes to compute the nce loss w/ candidate sampling.										
	...
	if (dynamic == False) :
		head_multi_vec = tf.reshape(tf.nn.embedding_lookup(example_emb, example_idxs), [batch_size, -1, model.embed_size])#batch, None, embed_size
		head_vec = tf.reduce_mean(head_multi_vec, 1, keep_dims=False)#batch, embed_size
		example_vec = head_vec + relation_vec
	if (dynamic == True) :
		head_multi_vec = tf.reshape(tf.nn.embedding_lookup(example_emb, example_idxs), [batch_size, -1, model.embed_size])#batch, K, embed_size
		head_lambda = tf.nn.embedding_lookup(user_lambda, example_idxs)
		head_p = tf.nn.embedding_lookup(user_p, example_idxs)
		relation_multi_vec = tf.reshape(tf.tile(relation_vec, [1, model.mult_size]), [-1, model.mult_size, model.embed_size])#batch, K, embed_size
		relation_cate_multi_vec = tf.reshape(tf.tile(relation_cate_vec, [1, model.mult_size]), [-1, model.mult_size, model.embed_size])#batch, K, embed_size
		example_multi_vec = head_multi_vec + relation_multi_vec#batch, K, embed_size
	...
	if (dynamic == True) :
		#print(list(range(model.mult_size)) * batch_size)
		user_cate_idxs = tf.cast(tf.convert_to_tensor(np.array(list(range(model.mult_size)), dtype=np.int64)), tf.int64)
		user_cate_idxs = tf.tile(user_cate_idxs, [batch_size])
		user_cate_idxs = user_cate_idxs + model.mult_size * tf.reshape(tf.tile(tf.expand_dims(example_idxs, 1), [1, model.mult_size]), [-1])
		user_cate = tf.nn.embedding_lookup(model.entity_dict['categories']['embedding'], tf.nn.embedding_lookup(model.cate_dict['user']['embedding'], user_cate_idxs))
		user_cate = tf.reshape(user_cate, [-1, model.mult_size, model.embed_size])
		attention_weight = tf.reduce_sum(user_cate * relation_cate_multi_vec, 2, keep_dims=False)#batch, K
		
		true_cate_idxs = tf.nn.embedding_lookup(model.cate_dict['product']['embedding'], label_idxs)
		true_cate_word_idxs = tf.gather(model.data_set.category_words, true_cate_idxs)
		true_cate_word_masks =  tf.gather(model.data_set.category_word_masks, true_cate_idxs)
		true_cate_emb, true_cw_embs = get_query_embedding(model, true_cate_word_idxs, true_cate_word_masks, model.entity_dict['word']['embedding'], reuse=True, scope=None) # get cate vectors
		true_cate = true_cate_emb				

		sampled_cate_idxs = tf.nn.embedding_lookup(model.cate_dict['product']['embedding'], sampled_ids)
		sampled_cate_word_idxs = tf.gather(model.data_set.category_words, sampled_cate_idxs)
		sampled_cate_word_masks = tf.gather(model.data_set.category_word_masks, sampled_cate_idxs)
		sampled_cate_emb, sampled_cw_embs = get_query_embedding(model, sampled_cate_word_idxs, sampled_cate_word_masks, model.entity_dict['word']['embedding'], reuse=True, scope=None) # get cate vectors
		sampled_cate = sampled_cate_emb
	...
	if (dynamic == False) :					
		...
		true_logits  = get_true_scores(model, example_multi_vec, true_w, true_b, head_lambda, head_p, model.selection, user_cate, true_cate, attention_weight)
		example_multi_vec = tf.reshape(example_multi_vec, [-1, model.embed_size])
		user_cate = tf.reshape(user_cate, [-1, model.embed_size])
		sampled_logits = get_scores(model, example_multi_vec, sampled_w, sampled_b_vec, head_lambda, head_p, model.selection, user_cate, sampled_cate, attention_weight)
	
	if (dynamic == False) :													
		return nce_loss(true_logits, sampled_logits), [example_vec, true_w, sampled_w]						
	if (dynamic == True) :													
		return nce_loss(true_logits, sampled_logits), [example_multi_vec, user_cate, true_w, sampled_w, true_cate, sampled_cate]
